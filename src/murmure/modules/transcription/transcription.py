...
from murmure.modules.vad import VADSegmentOutput
from murmure.utils.logger import get_logger

from murmure.utils.whispercpp_utils import get_whispercpp_model, ensure_whispercpp_installed
...
class TranscriptionModule:
    ...
    def __init__(self, 
                 model_dir_path : Path,
                 whispercpp_path : Path,
                 model_size: str = "base",
                 device : str = 'cpu'):
        
        self.model_size = model_size
        self.model_dir_path = os.path.join(str(model_dir_path), 'transcription')
        os.makedirs(str(self.model_dir_path), exist_ok=True)


        self.device = device

        if self.device == 'mps':
            self.binary_path = str(ensure_whispercpp_installed(whispercpp_path=whispercpp_path))
